...
<c:choose>
	...
	<div id="tt"
		style="color: black; font-family: Andale Mono; float: left; margin-left: 20px; margin-top: 30px; border: 1px solid silver; width: 1200px; background: white;">
...
<td><span style="color:orange">Czeka na akceptację przełożonego</span></td>
</c:when>
<c:when test="${s.status==2 }">
<td><span style="color:red">Odrzucone przez przełożonego</span></td>
</c:when>
<c:otherwise>

<td><span style="color:green">Zatwierdzony przez przełożonego</span></td>
</c:otherwise>
</c:choose> 
<c:url value="kklistawnioskow" var="idd">
<c:param name="ido" value="${s.idurlopu}"></c:param>

</c:url>	
		</td>
		<td align="left"> ${s.title }        </td>
		<td align="left">${s.dzial}</td>
		<td align="left">${s.rodzaj}</td>
		<td align="left">${s.zastepstwo}</td>
		<td align="left">${s.start }</td>
		<td align="left">${s.end}</td>
		<td align="left">${s.liczbadniurlopu}</td>
		<td><a href="${idd}">Usuń</a></td>
		</tr>
...
<c:url value="klistawnioskow1" var="tt">
