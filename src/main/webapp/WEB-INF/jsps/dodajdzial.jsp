...
<f:form modelAttribute="dzial" action="dodanodzial">

<table>
<tr><td>Nazwa działu:</td><td><f:input path="nazwa"/></td></tr>
<tr><td><input type="submit" value="dodaj"></td></tr>

</f:form>
<c:forEach items="${allDzial}" var="i">
<c:url value="edytujDzial" var="edit">
<c:param name="edytujDzial" value="${i.id}"></c:param>

</c:url>
<c:url value="usunDzial" var="deldzial">
<c:param name="usundzial" value="${i.id}"></c:param>

</c:url>

<tr><td><span style="color:blue">${i.nazwa}</span></td><td><a href="${edit }">edytuj</a></td><td><a href="${deldzial}">usuń</a></td></tr>
</c:forEach>
